...
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.clearProfilePanel = function () {
            const profileName = document.getElementById('profile-panel-name');
            const profileUsername = document.getElementById('profile-panel-username');
        ...
        window.loadUserProfile = function () {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    const response = yield fetch('/api/user/profile', {
                        credentials: 'include'
                    });
                    if (!response.ok) {
                    ...
                    // Load match history
                    yield loadMatchHistory(user.id);
                }
                catch (error) {
        ...
        window.loadOtherUserProfile = function (username) {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    const response = yield fetch(`/api/user/profile/${username}`, {
                        credentials: 'include'
                    });
                    if (!response.ok) {
                        console.error('Failed to fetch user profile');
        ...
        function loadMatchHistory(userId) {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    const response = yield fetch(`/api/matches/user/${userId}`, {
                        credentials: 'include'
                    });
                    if (!response.ok) {
        ...
        function renderMatchHistory(matches, userId) {
            const historyContainer = document.getElementById('profile-match-history');
            if (!historyContainer)
                return;
            historyContainer.innerHTML = '';
